...
- [x] **T-D05 (tooling)** Pre-commit forbidden-language hook (N8)
...
- [x] **T-099a** `<home>/.claude/housekeeper/lock.history` append-only
      JSONL (N6 carry-over)
...
- [x] **T-105** `diagnose` decoration with `falsePositiveSeenBefore: N`
  - Verify: fixture with one marker emits the field on the matching
    finding only
  - blockedBy: T-104

- [x] **T-106** `test/learning.test.mjs` — 12+ tests covering append
      paths, summary, JSON output, prune, false-positive marker, schema
  - blockedBy: T-100..T-105

- [x] **T-107** Performance bench: append cost < 5ms p99 on 10k-line
      JSONL
  - blockedBy: T-100
...
- [x] **T-200** `--mcp-command-rewrite=<old>=<new>` CLI parser
  - Verify: rejects malformed values; `--help` shows the flag

- [x] **T-201** `composeHardenPlan` extension for MCP rewrite mode
  - Verify: fixture with one broken MCP entry + valid new path produces
    a non-empty operations array
  - blockedBy: T-200

- [x] **T-202** Refusal classes: `mcp-rewrite-target-missing`,
...
- [x] **T-203** Acceptance card + fixture for MCP rewrite happy path
  - blockedBy: T-201

- [x] **T-204** `test/mcp-rewrite.test.mjs` — 10+ tests covering happy,
...
- [x] **T-300** `plugin.unused_past_grace` detector in `audit.mjs`
...
- [x] **T-301** `housekeeper prune` subcommand (audit-only)
...
- [x] **T-304** `test/plugin-prune.test.mjs` — 8+ tests including
...
- [x] **T-700** Schema-stability: add `learning` files + `lock.history`
      to `docs/schema-stability.md`; add `json-rewrite` (or new kinds
      per Q2) to documented mutation kinds. blockedBy: T-D04
- [x] **T-701a** Threat model addendum: learning-loop surface (PII,
      read/write trust boundary). blockedBy: T-D04
- [x] **T-701b** Threat model addendum: MCP rewrite (user-supplied path
