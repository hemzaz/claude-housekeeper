...
| 2026-05-11 | Q-USER-1 | Tie-break between equal-sized findings: lexicographic order of `targetPath` | Deterministic, cheap, testable. Surfaced by tie-breaker (`docs/design/clean-tie-breaker.md` §5). | user (Elad) |
| 2026-05-11 | Q-USER-2 | `composeCleanPlan` re-runs `assembleReport` internally rather than trusting the report passed in | ~100 ms cost prevents stale-report targeting (user runs `diagnose`, waits, then `clean`; report may be hours old). The freshness invariant is worth the cost. Surfaced by tie-breaker §5. | user (Elad) |
| 2026-05-11 | Q-USER-3 | `plugin.expected_orphan` never becomes cleanable (not in v0.2, not in v0.3+) | It's Claude Code's own contract per platform memo §8.2 R2 — concurrent sessions still hold that version. A user can't consent on behalf of their other live session. Surfaced by tie-breaker §5. | user (Elad) |
| 2026-05-11 | T-704 design | All 17 cross-memo conflicts resolved per tie-breaker rulings; buildable spec at `docs/design/clean-design.md` | Three peer memos (architect, product, platform) + a tie-breaker memo were synthesized into a final spec. T-704 implementation order is fixed: (1) two snapshot.mjs bug fixes (deletion-aware applyOperation + verify); (2) new audit detectors `plugin.cache_referenced_by_hook` and `housekeeper.stale_lock`; (3) new `scripts/lib/clean-plan.mjs`; (4) wire into `clean` handler. | synthesizer (Claude) |
